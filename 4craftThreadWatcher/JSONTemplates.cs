		...
		if(touched.Add(obj)) {
			...
			foreach(FieldInfo fi in fieldinfo) {
				...
				if(val) {
					...
					else Console.WriteLine("Null for this non-null object, property " + fi.Name + " of class " + obj.GetType().Name + ". Object type is " + fi.FieldType.Name);
				}
			}
			//Properties
			...
			foreach(PropertyInfo pi in propertyInfo) {
				...
				if(val) {
					...
					else Console.WriteLine("Null for this non-null object, property " + pi.Name + " of class " + obj.GetType().Name + ". Object type is " + pi.PropertyType.Name);
				}
			}
			return result;
		}
        Console.WriteLine("trying to save the same data twice");
